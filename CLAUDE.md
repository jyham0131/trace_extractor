# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

---

## 프로젝트 개요

장애 발생 시 로그 파일에서 거래 추적번호를 기반으로 관련 로그를 추출·분석하는 **tkinter 데스크탑 UI** 도구. PyInstaller로 exe 빌드 가능.

---

## 로그 포맷

```
[260426 21:56:17.725] [ 프로세스번호 ] [추적번호16자리] 데이터...
```

- 필드 0: 타임스탬프 `[YYMMDD HH:MM:SS.mmm]`
- 필드 1: 프로세스 번호 `[ NNNN ]`
- 필드 2: 추적번호 16자리 `[XXXXXXXXXXXXXXXX]`
- 필드 3~: 페이로드 데이터

### Xlog 파일 특이사항

- 파일명이 `Xlog`로 시작하는 파일(대소문자 무관)은 **payload** 안에 `TRACE_NO : <16자리추적번호>` 형식으로 릴레이 추적번호가 포함된다.
  - 예: `[260426 21:56:17.725] [kcppg11  ] [기존추적번호] MSG__REQ__CHK ▷ TRACE_NO : ABCD1234EFGH5678`
- 필드1은 일반 파일과 동일하게 프로세스 번호이며 릴레이 추적번호로 사용되지 않는다.

---

## 분석 흐름

```
step1 │ 입력 추적번호(들)
      │   → 전체 로그에서 field2(추적번호)가 일치하는 라인 추출
      ↓
step2 │ step1 결과 + keyword2(선택)
      │   → keyword2 포함 라인 중 Xlog 파일의 payload에서
      │     "TRACE_NO : XXXX" 패턴으로 새 추적번호 수집
      ↓
step3 │ step2에서 수집한 추적번호 + keyword3(선택)
      │   → 전체 로그에서 재검색
      │   → 매칭 라인 출력 / 미매칭 추적번호 분리 출력
```

### UI 입력 필드

| 필드 | 설명 |
|------|------|
| 1단계 · 추적번호 | 줄바꿈으로 구분, 여러 개 입력 가능 |
| 2단계 · 키워드 | Xlog TRACE_NO 수집 전 추가 필터 (비워두면 전체 Xlog 라인 대상) |
| 3단계 · 키워드 | 재검색 시 매칭 조건 (비워두면 추적번호만으로 매칭) |

---

## 아키텍처

```
trace_extractor/
├── app.py               # tkinter 메인 윈도우 (UI 진입점)
├── log_parser.py        # 로그 라인 파싱 (LogLine 데이터클래스)
├── trace_extractor.py   # step1/2/3 필터링 로직
├── xlog_handler.py      # Xlog payload의 TRACE_NO 추출
├── result_reporter.py   # AnalysisResult 데이터 구조
├── requirements.txt     # pyinstaller
└── build.bat            # exe 빌드 스크립트
```

### 핵심 모듈 책임

| 모듈 | 책임 |
|------|------|
| `log_parser.py` | 로그 한 줄 → `LogLine` 변환, 파일 전체 파싱 |
| `xlog_handler.py` | Xlog payload의 `TRACE_NO : ` 패턴에서 릴레이 추적번호 추출 |
| `trace_extractor.py` | step1/2/3 필터링 함수 |
| `result_reporter.py` | `AnalysisResult` 집계 (매칭/미매칭/카운트) |
| `app.py` | tkinter UI — 파일 선택, 키워드 입력, 결과 테이블 |

### LogLine 데이터 구조

```python
@dataclass
class LogLine:
    raw: str          # 원본 라인
    timestamp: str    # 타임스탬프
    field1: str       # 프로세스번호
    trace_id: str     # field2: 16자리 추적번호
    payload: str      # field3~: 페이로드
    source_file: str  # 파일명
    is_xlog: bool     # 파일명이 xlog로 시작하면 True
```

---

## 개발 환경 및 명령어

```bash
# 의존성 설치
pip install -r requirements.txt

# UI 실행
python app.py

# exe 빌드 (build.bat 또는 직접)
pyinstaller --onefile --windowed --name LogAnalyzer app.py
```

---

## 코딩 규칙

- **언어**: Python 3.9+, 타입힌트 필수
- **주석**: 한국어 (WHY 중심)
- **스타일**: 함수 단일 책임, 20줄 이하 권장
- **데이터 모델**: `dataclass` 사용, dict 직접 노출 지양
- **출력 구분**: 매칭 결과와 미매칭 결과는 반드시 분리 표시
